...
<div id="saveForm">
    ...
    <img src="<c:url value='/images/${product.board_Img}' />" alt="Product Image" style="border-radius:8px;border: 1px solid #dbcfcf;width: 400px;height: 400px;">
</div>
			...
			<div class="product-container">
				...
				<div><span class="title"> [${product.board_cate}]</span></div>
				<div><span class="title">  ${product.board_Title} </span></div>
				</div>
					 
						<div  style="border-bottom: 1px solid #dbcfcf;"></div>
						
					<div class="product-label">
					<div><span class="title">  ${product.user_nickname }</span></div>
					<div><span class="title"> ${product.board_Date}</span> </div> | <div>조회수:<span class="title"> ${product.board_Click}</span>  </div>
					</div>
					
		
					<div class="product-label">
					<div><span class="title"> ${product.loc_code}</span> </div> /<div><span class="title"> ${product.detail_loc}</span></div>
					</div>
					
					<div  style="border-bottom: 1px solid #dbcfcf;"></div>
					
					<div style=" margin-top: 35px; line-height: 1.5;">
    ...
    <p style="font-size: 13px; margin-left:150px; font-weight: bold; color: red;">이미 신청한 채팅입니다.</p>
</c:if>
    ...
    <form action="/testing/order">
		...
		<input type="hidden" name="sell_code" value="${product.user_code}" required>
			<div><span class="title">가격: <span style="color: #ff6f0f;">${product.board_Price}</span> 원</span></div><button type="submit">상품 구매</button>
	</form>
	
     
   <div class="chat_likes">
    <form action="/testing/checkCode" method="post">
        <input type="hidden" name="buy_code" value="${selectedUser[0].user_code}" required><br>
         <input type="hidden" name="sell_code" value="${product.user_code}" required><br>
         <input type="hidden" name="board_id" value="${product.board_Id}" required><br>
	...
	<c:if test="${owner}">
		...
		<form action="/testing/products/update" method="get">
			<div><span class="title">가격: <span style="color: #ff6f0f;">${product.board_Price}</span> 원</span></div>
			<input type="hidden" name="user_code1" id="user_code1"
				value="<%=selectedUser.getUser_code()%>" required> <input
				type="hidden" name="user_code3" id="user_code3"
